from bot_emision_facturas import bot, sessions
...
def manejar_callback(call):
    ...
    if data.startswith("edit:"):
        ...
        sessions[chat_id]["await_edit_row"] = indice_fila
...
def recibir_texto_nueva_fila(mensaje):
    ...
    _finish_add_row(chat_id, item, bot, sessions, debounced_write=write_sheet_data)
...
def recibir_texto_editar_fila(mensaje):
    ...
    """
    chat_id = mensaje.chat.id
    indice_fila = sessions.get(chat_id, {}).get("await_edit_row")
    ...
    sessions[chat_id]["items"][indice_fila] = item
    _mark_dirty(chat_id, sessions)
    actualizar_tabla_items(chat_id)

    sessions[chat_id].pop("await_edit_row", None)
...
def actualizar_tabla_items(chat_id):
    ...
    bot.edit_message_text(nuevo_md, chat_id, sessions[chat_id]["message_id"],
...
def registrar_manejadores_items(bot):
    ...
    """
    bot.message_handler(commands=['items'])(mostrar_items)
    bot.callback_query_handler(func=lambda call: call.data.startswith(("edit:", "addrow", "delete:")))(manejar_callback)
    bot.message_handler(func=lambda msg: sessions.get(msg.chat.id, {}).get("await_new_row"))(recibir_texto_nueva_fila)
    bot.message_handler(func=lambda msg: sessions.get(msg.chat.id, {}).get("await_edit_row") is not None)(recibir_texto_editar_fila)